		...
		success:function(res){
			...
			for(var i=0;i<res.length;i++){
				...
				if(res[i].id == idNum){
					...
					$(".pic").find("img").attr("src",res[i].url);
					
					//放大镜大图的图片路径
					
					$(".pic_big").find("img").attr("src",res[i].url);
					
					//购买页的标题
					
					$(".con_text_title").find("a").html(res[i].douhao);
					
					//现在购买的价格
					
					$(".pro_price").find("span").html(res[i].buy_price);
					
					//原来的价格
					
					$(".dele_price").html(res[i].buy_del);
					
					//名字
					
					$(".product_Name").html(res[i].des02);
					
					//商品介绍
					
					$(".product_details").html(res[i].des03+'(<span>'+res[i].des04+'</span>)');
					
					//立即购买按钮添加goods（id）
					
					$("#buy_rightNow").attr("goods",res[i].id)
					...
					for(var j=0;j<res[i].picArr.length;j++){
						
						html +='<li><img src="'+res[i].picArr[j]+'" alt="" /></li>';
...
function xidingdaohang(){
	...
	//页面滑动到参数，详情，评价区域，下面线条自动移动
	$(window).scroll(function(){
//		console.log($(".reviews").offset().top);
		//console.log($(".pro_nav_line"));
//		console.log($(document).scrollTop());
		//console.log($(".pro_main_cont_nav").children().find("li:first").width()*2);
		if($(document).scrollTop() <= ($(".details").offset().top-60)){

			$(".pro_nav_line").stop().animate({
				"left":$(".pro_main_cont_nav").children().find("li:first").width()*0 
			})
		
		}else if($(document).scrollTop() <= ($(".reviews").offset().top-60)){
			
			$(".pro_nav_line").stop().animate({
			
				"left":$(".pro_main_cont_nav").children().find("li:first").width()*1
			...
			$(".pro_nav_line").stop().animate({
				...
				"left":$(".pro_main_cont_nav").children().find("li:first").width()*2
